    ...
    return (
        <Wrapper className='relative bg-gradient-to-b from-violet-500/10  h-fit pb-14 rounded-lg border border-violet-500/40 shadow-btnGlow'>
            <div className='w-full flex items-center justify-between p-8'>
                <Button onClick={handleSlideTwo} disabled={slide === 1} variant={slide === 1 ? "ghost" : "default"}><Icons.leftArrow className='h-4 w-4 mr-1' /> Back</Button>
                <Button onClick={handleSlide} disabled={slide === data.length} variant={slide === data.length ? "ghost" : "default"}>Next <Icons.rightArrow className='h-4 w-4 ml-1' /></Button>
            </div>
